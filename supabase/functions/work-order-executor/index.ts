...
async function validateQAToolCalls(supabase: any, workOrderId: string): Promise<{ valid: boolean; error?: string }> {
  try {
    // Check if this is a remediation WO with unresolved findings
    ...
    if (woError) {
      console.error('Failed to fetch WO for QA validation:', woError);
      return { valid: true }; // Fail open to avoid blocking non-remediation WOs
    }
    
    // Only validate remediation WOs
    const isRemediation = wo?.tags?.includes('remediation');
    if (!isRemediation) {
      return { valid: true };
    }
    
    // Check for unresolved qa_findings
    ...
    if (findingsError) {
      console.error('Failed to fetch qa_findings:', findingsError);
      return { valid: true }; // Fail open
    }
    
    const hasUnresolvedFindings = findings && findings.length > 0;
    if (!hasUnresolvedFindings) {
      return { valid: true }; // No findings to resolve
    }
    
    // Check execution log for resolve_qa_findings tool call
    const { data: toolCalls, error: logError } = await supabase
    ...
    if (logError) {
      console.error('Failed to fetch execution log:', logError);
      return { valid: true }; // Fail open
    }
    
    // Search for resolve_qa_findings or update_qa_checklist tool calls
    const hasResolveCall = toolCalls?.some((log: any) => 
      log.detail?.tool_name === 'resolve_qa_findings' && 
      log.detail?.event_type === 'tool_result'
    );
    
    const hasUpdateCall = toolCalls?.some((log: any) => 
      log.detail?.tool_name === 'update_qa_checklist' && 
      log.detail?.event_type === 'tool_result'
    );
    
    if (!hasResolveCall && !hasUpdateCall) {
      return { 
        valid: false, 
        error: 'Remediation WO has unresolved QA findings but resolve_qa_findings and update_qa_checklist tools were not called. You must actually invoke these tools, not just claim you did.' 
      };
    }
    
    return { valid: true };
  } catch (e: any) {
    console.error('QA tool validation exception:', e);
    return { valid: true }; // Fail open to avoid blocking legitimate completions
  }
}
