...
// WO-0474: Fix catch block bypassing stall detection ÃÂ¢ÃÂÃÂ API error counter + emergency trim + non-retryable fail-fast
...
// WO-0378: Message corruption fix ÃÂ¢ÃÂÃÂ repairMessages, dispatchTool try/catch, pair-safe trimming
// WO-0187: Continuation pattern ÃÂ¢ÃÂÃÂ checkpoint before timeout, self-reinvoke via pg_net
...
const TIMEOUT_MS = 380_000; // 380s Ã¢ÂÂ 20s buffer before 400s Supabase Pro wall clock limit (waitUntil mode)
const CHECKPOINT_MS = 350_000; // 350s Ã¢ÂÂ checkpoint before timeout to enable continuation
...
const DEFAULT_MODEL = "claude-opus-4-6"; // Fallback only ÃÂ¢ÃÂÃÂ prefer agent_execution_profiles.model
...
function repairMessages(messages: Array<{ role: string; content: any }>): number {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (!nextMsg || nextMsg.role !== 'user' || !Array.isArray(nextMsg.content)) {
      // No matching user message with tool_results ÃÂ¢ÃÂÃÂ inject one
...
export async function runAgentLoop(
  ...
  // WO-0401: Config-driven model ÃÂ¢ÃÂÃÂ profile > WO override > default
  ...
  while (true) {
    ...
    if (messages.length > maxMessages) {
      ...
      // its preceding assistant message has tool_use blocks ÃÂ¢ÃÂÃÂ keep the pair together.
      ...
      if (cutIdx < messages.length) {
        ...
        if (msgAtCut.role === 'user' && Array.isArray(msgAtCut.content) &&
          ...
          // This is a tool_result message ÃÂ¢ÃÂÃÂ its assistant pair is at cutIdx-1
    ...
    try {
      ...
      // Successful API call ÃÂ¢ÃÂÃÂ reset error counter
      ...
      if (response.stop_reason === "end_turn") {
        ...
        // No terminal tool was called ÃÂ¢ÃÂÃÂ nudge the model
      ...
      // Non-retryable: prompt too long ÃÂ¢ÃÂÃÂ trim aggressively or fail immediately
      if (e.message?.includes('prompt is too long') || e.status === 400) {
        ...
        if (messages.length > emergencyMax) {
          ...
          // Already minimal ÃÂ¢ÃÂÃÂ context is fundamentally too large, fail immediately
          const reason = `Fatal: prompt too large (${e.message}). Cannot trim further ÃÂ¢ÃÂÃÂ system prompt + initial context exceeds model limit.`;
  ...
  // Should not reach here ÃÂ¢ÃÂÃÂ loop exits via checkpoint, terminal tool, or stall detection
