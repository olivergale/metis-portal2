...
// WO-0186: Bypass guard ÃÂ¢ÃÂÃÂ non-master agents cannot use set_config to bypass enforcement
 ...
 * NOTE: run_sql wraps query in SELECT jsonb_agg(...) ÃÂ¢ÃÂÃÂ DDL will fail silently.
 ...
 * Uses EXECUTE directly ÃÂ¢ÃÂÃÂ DDL persists correctly.
...
export async function handleApplyMigration(
  ...
  try {
    ...
    // Uses run_sql_void (EXECUTE directly) ÃÂ¢ÃÂÃÂ run_sql wraps in SELECT subquery which breaks DDL.
    ...
    if (!ddlOk || error) {
      if (error && (error.includes("lock timeout") || error.includes("could not obtain lock"))) {
        return { success: false, error: `Migration blocked ÃÂ¢ÃÂÃÂ another agent is running DDL. Try again in a few seconds.` };
      }
