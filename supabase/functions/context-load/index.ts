...
// v16: Fix doc_status ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ use project UUID (not code string) for project_documents query + call check_doc_currency() RPC
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const results = await Promise.all([
      ...
      // 6. Directives (system rules) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ exclude portal_only directives (v13)
    ...
    const verificationRequirements = {
      ...
      enforcement_transition: 'in_progress ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ review',
