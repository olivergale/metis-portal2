  ...
  try {
    ...
    const funcNames = [...new Set(
      (combined.match(/\b([a-z][a-z0-9_]{2,50})\s*\(/g) || []).map(m => m.replace(/\s*\($/, ''))
        .filter(n => n.includes('_')) // Must have underscore ÃÂ¢ÃÂÃÂ filters "fail(", "has(", "set(" etc
