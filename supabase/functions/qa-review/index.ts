...
async function fetchGitHubFileEvidence(paths: string[], supabase: any, acText?: string): Promise<GitHubFileEvidence[]> {
  ...
  for (const p of paths.slice(0, 3)) {
    try {
      ...
      // Run pattern matching on FULL file content (not just snippet)
      let patternResult: { matched_patterns: string[]; unmatched_patterns: string[] } | undefined;
      if (acText) {
        patternResult = checkCodePatternsInFile(decoded, acText);
      }
      results.push({
        path: p,
        exists: true,
        size: data.size || decoded.length,
        content_snippet: decoded.slice(0, 2000),
        matched_patterns: patternResult?.matched_patterns,
        unmatched_patterns: patternResult?.unmatched_patterns,
      });
    } catch (e: any) {
...
${(() => {
    ...
    return githubFileEvidence.map(gf => {
      ...
      return `File: ${gf.path} - EXISTS (${gf.size} bytes)${patterns.length > 0 ? '\\n  ' + patterns.join('\\n  ') : ''}\\n  Snippet (first 2000 chars): ${gf.content_snippet?.slice(0, 500) || 'N/A'}`;
    }).join('\\n');
  })()}

## MUTATION VERIFICATION (lie detector - claimed mutations vs actual DB state)
${(() => {
    if (!mutationVerification) return 'Mutation verification not run or no mutations recorded.';
    const mv = mutationVerification;
    let summary = 'Total mutations checked: ' + (mv.total_checked || 0) + '\\nVerified (exist in DB): ' + (mv.total_verified || 0) + '\\nFailed (claimed but not found): ' + (mv.total_failed || 0);
    if (mv.failed_details && mv.failed_details.length > 0) {
      summary += '\\n\\nFAILED MUTATIONS (claimed but verification query returned no rows):';
      for (const fail of mv.failed_details) {
        summary += '\\n- ' + fail.tool_name + ': ' + fail.action + ' ' + fail.object_type + ' ' + fail.object_id;
        summary += '\\n  Reason: ' + fail.reason;
        summary += '\\n  Query: ' + fail.verification_query;
      }
    }
    return summary;
  })()}
