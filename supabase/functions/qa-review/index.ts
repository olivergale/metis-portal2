  ...
  try {
    ...
    const funcNames = [...new Set(
      (combined.match(/\b([a-z][a-z0-9_]{2,50})\s*\(/g) || []).map(m => m.replace(/\s*\($/, ''))
        .filter(n => n.includes('_')) // Must have underscore ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ filters "fail(", "has(", "set(" etc
...
If the summary says "created table X", "applied migration Y", "inserted Z rows" ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ there MUST be EITHER a matching tool call in the execution log OR the entity must exist in system state evidence. Example: claim "created function foo()" is SUPPORTED if system state shows "Function foo: EXISTS in pg_proc", even without an execution_log entry.

### CHECK 3: "Deployed" / "Verified" Statements
If the summary says "deployed function X" or "verified endpoint Y" ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ there MUST be a deploy_edge_function call or a verification query in the log. Unsubstantiated deploy/verify claims = FABRICATED.
